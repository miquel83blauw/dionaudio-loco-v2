 ...
 *
 * Author:      Miquel Blauw <info@dionaudio.nl> *              Copyright 2016
 *
...
static int snd_rpi_dionaudio_loco-v2_init(struct snd_soc_pcm_runtime *rtd)
{
	if (digital_gain_0db_limit)
	{
		...
		ret = snd_soc_limit_volume(codec, "Digital Playback Volume", 207);		if (ret < 0)
...
static int snd_rpi_dionaudio_loco-v2_hw_params(struct snd_pcm_substream *substream,
...
/* machine stream operations */static struct snd_soc_ops snd_rpi_dionaudio_loco-v2_ops = {
...
static int snd_rpi_dionaudio_loco-v2_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
	    ...
	    struct snd_soc_dai_link *dai = &snd_rpi_dionaudio_loco-v2_dai[0];
